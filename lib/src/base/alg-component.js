...
class AlgComponent extends HTMLElement {
  ...
  constructor() {
    ...
    /** @type {String | Object} */
    this.controller = null; // Controller associate to the component

    /** @type {String} */
    this.hash = (++AlgComponent.hashIndex).toString().padStart(4, '0'); // universal id

    /** @type {Map<String, HTMLElement>} */
    this.ids = null; // children ids defined in createShadowElement.

    /** @type {Boolean} */
    this.loadedComponent = false; // True if component inserted in a stable dom

    /** @type {Number} */
    this._oldTabIndex = null; // cache for tabIndex when disabled
  ...
  attributeChangedCallback(attrName, oldVal, newVal) {
    ...
    if (newVal == null && this.attributeManager.isRemoved(name)) return; // happens in attribute removal

    if (newVal == null || this.attributeManager.isSubscriptionTried(attrName)) {
      this.attributeManager.change(name, newVal);
    } else {
  ...
  get templateInfo() { // TODO: move to TemplateManager
   ...
   */
  restoreTemplate() { // TODO: move to TemplateManager
