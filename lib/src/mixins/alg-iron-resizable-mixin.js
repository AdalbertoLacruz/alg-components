...
import { EventManager } from '../base/event-manager.js';
 ...
 * @param {*} base
 */
export const AlgIronResizableMixin = (base) => class extends base {
  /** @override */
  deferredConstructor() {
    ...
    this.attributeManager
      ...
      .on((parentResizable) => {
        if (parentResizable) {
          // @ts-ignore
          EventManager.windowEvm.unsubscribeMe(this, 'resize');
        }
      });

    this.eventManager
      // listen children resizables
      .onLink('iron-request-resize-notifications', (event, context) => {
      ...
      }) // important! to stop bubble and sequential exec after fireEvent
