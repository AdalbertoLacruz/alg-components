...
export const AlgPaperRippleMixin = (base) => class extends base {
  ...
  deferredConstructor() {
    ...
    this.attributeManager
      ...
      .define('noink', TYPE_BOOL, { isLocal: true, alias: ['ink-noink'] })
      .reflect();

    this.eventManager
      .define('transitionend', new ObservableEvent('transitionend').setType(TYPE_EVENT))
      .on('down', this.ensureRipple.bind(this)) // a mouse event has x,y
      .on('focused', this.ensureRipple.bind(this))
      .subscribe();

    // this.messageManager
    //   .from('ensure-ripple', (event) => this.ensureRipple(event));
  }
  ...
  ensureRipple(event = null) {
    if (!this.hasRipple()) {
      ...
      if (event) {
        Promise.resolve().then(() => this._ripple.uiDownAction(event));
      }
    }
  }
