...
import { TYPE_NUM } from '../util/constants.js';
...
class AlgPaperButtonBehavior extends
  ...
  deferredConstructor() {
    ...
    attributeManager
      ...
      .store((entry$) => { this.active$ = entry$; }) // redundant, to avoid warnings

      .onChange('disabled', this._calculateElevation.bind(this))

      .define('elevation', TYPE_NUM, { value: 0})
      .reflect()
      .store((entry$) => { this.elevation$ = entry$; });
