...
import { AlgLog } from '../base/alg-log.js';
...
class Observable {
  ...
  constructor(name = '') {
    ...
    /** @type {Number} Increments for each value changed, if not null */
    this.changed = null;

    /** @type {*} Last value received when delayed == true */
    this.delayedValue = null;

    /** @type {Boolean} true, avoid update */
    this.disabled = false;

    /** @type {Boolean} True if dispatch has been executed at lest once */
  ...
  set value(value) {
    AlgLog.log(`${this.prefix}#${this.name}`, value);
  ...
  */
   ...
   */
  log(message) {
    AlgLog.log(`${this.prefix}#${this.name}`, message);
  }
