...
import { TYPE_STRING } from '../src/util/constants.js';
...
class AlgPaperFab extends AlgPaperButtonBehavior {
  ...
  deferredConstructor() {
    ...
    this.attributeManager
      .define('aria-label', TYPE_STRING)
      .reflect()

      // Specifies the icon name or index in the set of icons available in
      // the icon's icon set. If the icon property is specified,
      // the src property should not be.
      .define('icon', TYPE_STRING, { countChanges: true })
      .on((value) => {
      ...
      .define('label', TYPE_STRING, { countChanges: true })
      ...
      .define('src', TYPE_STRING, { countChanges: true })
      .on((value) => {
  ...
  _computeIsIconFab() {
    if (!this.attributeManager.hasChanged('iconFab', ['icon', 'label', 'src'])) return;
    ...
    AttributeManager.attributeToggle(this.ids['span'], 'hidden', spanHiden);
    AttributeManager.attributeToggle(this.ids['icon'], 'hidden', !spanHiden);
  }
}
