...
class AlgPaperRadioButton extends AlgPaperCheckedElementBehavior {
  ...
  /**
   * css Calculate --paper-radio-button-ink-size
  ...
  calculateInkSize(id, options) {
    ...
    const inkSize = Rules.getComputedProperty(this, '--paper-radio-button-ink-size');
    if (!inkSize) {
      const size = parseFloat(Rules.getComputedProperty(this, '--paper-radio-button-size') || '16px');
